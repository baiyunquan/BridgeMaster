...
### 模块 1：房间与流程编排（LobbyManager）

职责：

1. 维护房间生命周期、邀请码、成员和心跳。
2. 管理普通模式与辅助模式状态机。
3. 通过事件总线向 SSE 输出快照与增量事件。

### 模块 2：接口网关（server.ts）

职责：

1. 暴露大厅、牌局、辅助模式、SSE、记录查询接口。
2. 对请求参数做解析与校验。
3. 在关键节点触发结果落盘逻辑（普通模式完局、辅助模式完局）。

### 模块 3：结果记录（GameRecordLogger）

职责：

1. 以 JSON Lines 形式记录每局结果。
2. 记录普通模式 completed/aborted。
3. 记录辅助模式 completed 结果，包含操作位、定约、局况、出牌总数等字段。
